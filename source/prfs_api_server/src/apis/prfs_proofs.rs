...
pub async fn create_prfs_proof(
    ...
    if msg != input.proof_sig_msg {
        let resp = ApiResponse::new_error(
            &PRFS_API_ERROR_CODES.NOT_MACHING_SIG_MSG,
            format!("msg: {:?}, author_sig_msg: {:?}", msg, input.proof_sig_msg),
        );
        return (StatusCode::BAD_REQUEST, Json(resp));
    }

    if let Err(err) = verify_eth_sig_by_pk(&input.proof_sig, &msg, &input.proof_public_key) {
        let resp = ApiResponse::new_error(
            &PRFS_API_ERROR_CODES.INVALID_SIG,
            format!("sig: {}, err: {}", input.proof_sig, err),
        );
        return (StatusCode::BAD_REQUEST, Json(resp));
    }
    ...
    let proof = PrfsProof {
        ...
        public_key: input.proof_public_key.to_string(),
