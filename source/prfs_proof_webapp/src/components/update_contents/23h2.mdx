...
- `appSignIn` API is replaced with `proofGen` API. AppSignIn functionality is served via
  query parameters under `proofGen`.

## 2024-02-16

### Prfs Proof

- Major refactor in circuit input data structure. Circuit input variants implement the interfaces
...
- Refactor `prfs-proof-webapp` to become a multi-tenant application: `prfs-proof-webapp`,
  `prfs-console-webapp`.

### Prfs Id

- Sunset localStorage-based message between windows for security reason. (Check out ITP "tracking
  prevention" of WebKit). Instead, we do this using websocket.
