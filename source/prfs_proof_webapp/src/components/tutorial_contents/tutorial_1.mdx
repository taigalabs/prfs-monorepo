import styles from '@/components/tutorial/TutorialMarkdown.module.scss';
...
you are done with each step and follow the orange 
dots. <span className={styles.orangeDot}></span>
...
Getting f(2) = 4 is trivial. But if we ask f^-1(y) = 8, the argument that makes the result to be 8,
it's not so obvious how we should obtain the solution, which is 3.
